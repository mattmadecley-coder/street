...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  let filtered = products.filter((product) => {
    const searchable = `${product.title} ${product.category} ${product.colors.join(" ")} ${product.sizes.join(" ")} ${product.tags.join(" ")} Seventy Four Uniform`.toLowerCase();
    ...
    return matchesQuery && matchesCategory && matchesColor && matchesSize && matchesAvailability && matchesPrice;
  });
