...
export default function BrandApplicationPage() {
  ...
  if (status === "sent") return (
    <main className="application-shell">
      <div className="application-success"><p className="eyebrow">Application received</p><h1>Your next audience could find you here.</h1><p>We review every application carefully. If your label is a strong fit, the Street team will contact you within 3–5 business days to discuss how we can help more shoppers discover your products and reach your store.</p><Link href="/" className="cta"><span>Back to Street</span><span>→</span></Link></div>
    </main>
  );
  ...
  return (
    <main className="application-shell">
      <div className="application-intro"><Link href="/" className="wordmark">STREET</Link><p className="eyebrow">Grow your brand with Street</p><h1>Reach more streetwear shoppers.</h1><p>Street helps independent labels build awareness, grow their following, send qualified shoppers to their store, and create more opportunities for sales. We review each brand for originality, product quality, consistency, and a meaningful contribution to the streetwear community.</p></div>
      <form className="application-form" onSubmit={submit}>
        <label>Brand name<input required autoComplete="organization" {...field("brandName")} /></label>
        <div className="form-grid"><label>Your name<input required autoComplete="name" {...field("applicantName")} /></label><label>Your role<input required autoComplete="organization-title" placeholder="Founder, marketing, manager…" {...field("role")} /></label></div>
        <label>Brand website<input required type="url" inputMode="url" autoComplete="url" placeholder="https://" {...field("website")} /></label>
        <label>Brand Instagram<input required inputMode="url" autoCapitalize="none" autoCorrect="off" placeholder="@yourbrand or profile URL" {...field("instagram")} /></label>
        ...
        <div className="form-grid"><label>Phone number<input required type="tel" inputMode="tel" autoComplete="tel" {...field("phone")} /></label><label>Email<input required type="email" inputMode="email" autoComplete="email" autoCapitalize="none" {...field("email")} /></label></div>
        <label>What should shoppers know about your brand?<textarea rows={5} placeholder="Tell us about your story, community, releases, customer, and what makes the label original." {...field("notes")} /></label>
