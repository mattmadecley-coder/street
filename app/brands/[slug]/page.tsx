...
export default async function BrandPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<Params> }) {
  ...
  const catalog = await getCatalogPage({ brand: slug, page: currentPage, availability: filters.availability, sort: filters.sort });
      ...
      <div className="shell">
        ...
        <section className={styles.catalog}>
          ...
          <form className="filters" action={`/brands/${slug}`}>
            ...
            <select name="sort" defaultValue={filters.sort ?? ""}><option value="">Newest</option><option value="price-low">Price: low</option><option value="price-high">Price: high</option></select>
            <button type="submit">Apply</button>
          </form>
          {products.length ? <div className="grid">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="empty"><p>No products match those filters.</p><Link className="link-small" href={`/brands/${slug}`}>Reset filters</Link></div>}
