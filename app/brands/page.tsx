      ...
      <div className="shell">
        ...
        <div className="brand-directory">
          {STREET_BRANDS.map((brand) => {
            ...
            return <Link key={brand.slug} href={`/brands/${brand.slug}`} className="brand-directory-card"><span className="brand">Brand</span><strong>{brand.name}</strong><span>{count} pieces</span></Link>;
          })}
        </div>
      </div>
    </main>
