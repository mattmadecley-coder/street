    ...
    <main>
      ...
      <div className="shell">
        ...
        <div className={styles.directory}>
          {brands.map((brand) => {
            const body = <><div className={styles.logoArea}>{brand.logoUrl ? <img src={brand.logoUrl} alt={brand.name} /> : <strong>{brand.name}</strong>}</div><span>{brand.productCount ? `${brand.productCount} pieces` : "Catalog coming soon"}</span>{instagramHandle(brand.instagramUrl) ? <p className={styles.instagram}>@{instagramHandle(brand.instagramUrl)}</p> : null}</>;
