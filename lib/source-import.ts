...
async function json<T>(url: string): Promise<T | null> { try { const response = await fetch(url, { cache: "no-store", headers }); return response.ok ? JSON.parse(await response.text()) as T : null; } catch { return null; } }
async function handles(base: string) { try { const response = await fetch(`${base}/collections/all`, { cache: "no-store", headers }); if (!response.ok) return []; const items = new Set<string>(); for (const match of (await response.text()).matchAll(/\/products\/([a-z0-9-]+)/gi)) items.add(match[1]); return [...items].slice(0, 120); } catch { return []; } }
